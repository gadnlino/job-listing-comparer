...
The system SHALL support deploying the FastAPI app alone (without docker-compose llama sidecar or WeasyPrint) on memory-constrained hosts such as Railway free tier (~512 MB), using Groq for executive summaries and browser PDF generation. Public discovery SHALL use a static GitHub Pages landing page that links to the Railway production app.
...
#### Scenario: GitHub Pages links to production app

- **WHEN** the GitHub Pages landing site is published
- **THEN** documentation records both the Pages URL (`https://<owner>.github.io/job-listing-comparer/`) and the Railway production URL (`https://job-listing-comparer-production.up.railway.app/`), with the landing page CTA pointing to Railway
